			...
			if Propertys[Number] then
				for Key in pairs(Interiors) do
					for Index in pairs(Interiors[Key]) do
						if #(Coords - Interiors[Key][Index]["Exit"]) <= 1.75 then
							Nearest = { Action = "Exit", Number = Number, Coords = Interiors[Key][Index]["Exit"] }
						else
							if Nearest and Nearest.Action == "Exit" then
								if Coords["z"] < Nearest.Coords["z"]-20.0 then
									SetEntityCoords(Ped,Nearest.Coords["x"],Nearest.Coords["y"],Nearest.Coords["z"]-1.0,false,false,false,false)
								end
							end
						end
					end
				...
				for Number,Value in pairs(Propertys) do
					if #(Coords - Value.Coords["xyz"]) <= 1.75 then
						Nearest = { Action = "Enter", Number = Number, Coords = Value.Coords["xyz"] }
					else
						if Nearest then
							if #(Coords - Nearest.Coords) > 1.75 then
								Nearest = nil
							end
						end
					end
