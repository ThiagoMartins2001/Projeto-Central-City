...
-- THREADBUTTON
-----------------------------------------------------------------------------------------------------------------------------------------
local PoliceRadar = true
local PoliceFreeze = true
-----------------------------------------------------------------------------------------------------------------------------------------
-- CLOSESYSTEM
-----------------------------------------------------------------------------------------------------------------------------------------
...
CreateThread(function()
	while true do
		...
		if IsPedInAnyPoliceVehicle(Ped) and LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Prf"] or LocalPlayer["state"]["Policia"] then
			if PoliceRadar then
...
RegisterCommand("toggleFreeze",function()
	local Ped = PlayerPedId()
	if IsPedInAnyPoliceVehicle(Ped) and LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Prf"] or LocalPlayer["state"]["Policia"] and not IsPauseMenuActive() then
...
RegisterCommand("enterTencodes",function()
	if LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Prf"] or LocalPlayer["state"]["Policia"] and LocalPlayer["state"]["Route"] < 900000 and not IsPauseMenuActive() then
...
--[[RegisterKeyMapping("toggleRadar","Ativar/Desativar radar das viaturas.","keyboard","J")
