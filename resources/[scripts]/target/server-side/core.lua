...
AddEventHandler("target:Gymnasium",function(Value)
	...
	if Passport and not Gymnasium["Gymnasium"..Value.Location..":"..Value.Equipment..""..Value.Position] then
		...
		while vTASKBAR.Task(source, 5, 10000) do
			TriggerClientEvent("Progress", source, 500)
			vRP.SetWeight(Passport, 10)  -- Peso fixo de 10
...
AddEventHandler("target:PaymentGym", function(Location, Price)
    local source = source
    print("Evento target:PaymentGym acionado para o jogador: " .. source)  -- Depuração
    
    -- Supondo que a função que recupera o Passport seja vRP.Passport(source)
    local Passport = vRP.Passport(source)
    print("Valor do Passport para o jogador " .. source .. ": " .. tostring(Passport))  -- Depuração

    -- Verifique se o Passport é válido
    if Passport then
        local Data = vRP.UserData(Passport, "Gymnasium")
        print("Dados de Gymnasium para o jogador " .. source .. ": " .. json.encode(Data))  -- Depuração
        
        -- Verifique se o jogador já tem uma assinatura válida para a academia
        if not Data[tostring(Passport)] or (Data[tostring(Passport)] <= os.time()) then
            -- Verifica se o jogador possui um grupo Premium ou se aceitou o pagamento
            print("Verificando grupo Premium ou aceitando pagamento")  -- Depuração
            if vRP.HasGroup(Passport, "Premium") or vRP.Request(source, "Academia", "Assinar Academia por <b>$" .. parseFormat(Price) .. "</b>? Lembrando que a duração do mesmo é de 7 dias.") then
                print("Pedido de pagamento aceito, processando pagamento...")  -- Depuração
                
                local paymentSuccess = vRP.PaymentFull(Passport, tonumber(Price), "Academia")
                print("Pagamento realizado: " .. tostring(paymentSuccess))  -- Depuração
                
                if paymentSuccess then
                    -- Se o pagamento foi realizado, define a duração da assinatura de 7 dias (604800 segundos)
                    Data[tostring(Passport)] = os.time() + 604800  -- 7 dias em segundos
                    vRP.Query("playerdata/SetData", {Passport = Passport, dkey = "Gymnasium", dvalue = json.encode(Data)})
                    
                    -- Envia a notificação para o jogador
                    ...
                    print("Falha no pagamento")  -- Depuração caso o pagamento falhe
                end
            else
                print("Solicitação de pagamento não aceita ou falhou.")  -- Depuração caso o Request falhe
            end
        else
            -- Caso o jogador já tenha uma assinatura válida
            TriggerClientEvent("target:Gymnasium", source, {Gymnasium, Data})
        end
    else
        print("Passport não encontrado para o jogador: " .. source)  -- Depuração se Passport não for encontrado
    end
end)
