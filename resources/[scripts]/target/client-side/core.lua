...
CreateThread(function()
	...
	AddCircleZone("CityHall",vec3(-551.93,-190.04,38.22),0.5,{
		name = "CityHall",
	},{
		Distance = 1.75,
		options = {
	...
	AddCircleZone("ElectricityRestore", vec3(2137.08,1936.64,93.77), 0.5, {
		name = "ElectricityRestore",
		heading = 0.0
	}, {
		Distance = 0.75,
		options = {
			{
				event = "inventory:ElectricityRestore",
				label = "Restabelecer Energia",
				tunnel = "server"
			}
		}
	})
	...
	AddCircleZone("Pdm",vec3(562.54,-242.05,49.98),0.75,{
		name = "Pdm",
		heading = 0.0
	},{
		Distance = 2.0,
...
local Fuels = {
	...
	vec3(212.23,-889.33,29.92),
	vec3(210.51,-894.21,29.92),
	vec3(211.53,-894.64,29.92),
	vec3(213.20,-890.04,29.92),
	vec3(206.35,-887.36,29.92),
	vec3(204.62,-892.21,29.92),
	vec3(203.59,-891.65,29.92),
	vec3(205.31,-886.88,29.92),
	vec3(199.45,-884.65,29.92),
	vec3(197.69,-889.46,29.92),
	vec3(196.61,-889.07,29.92),
	vec3(198.38,-884.18,29.92),
	vec3(2636.16,266.14,100.23),
	vec3(1505.45,773.44,78.43),
	vec3(2926.50,4160.35,52.50),
	vec3(-3020.40,406.61,16.64)
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- TARGETENABLE
-----------------------------------------------------------------------------------------------------------------------------------------
						...
						if #(Coords - entCoords) <= 1.0 and Plate ~= "PDMSPORT" then
							...
							if not Combustivel then
								if GetSelectedPedWeapon(Ped) == 883325847 and GetVehicleFuelLevel(entityHit) < 99.0 then
									...
									if LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Prf"] then
									...
									if Entity(entityHit)["state"]["Tow"] then
										...
										if GetEntityModel(Tower) == GetHashKey("flatbed3") and vehModel ~= GetHashKey("flatbed3") then
