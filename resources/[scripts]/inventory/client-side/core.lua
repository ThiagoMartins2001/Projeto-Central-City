...
RegisterNUICallback("Send", function(Data, Callback)
	if not IsPedArmed(PlayerPedId(), 6) and MumbleIsConnected() then
		vSERVER.Send(Data["slot"], Data["amount"], Data["key"])
	end
	Callback("Ok")
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand("Inventory", function()
	local Ped = PlayerPedId()
	if not IsPauseMenuActive() and GetEntityHealth(PlayerPedId()) > 100 and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not LocalPlayer["state"]["Carry"] and not IsPlayerFreeAiming(PlayerId()) and not LocalPlayer["state"]["inArena"] then
		if IsPedInAnyVehicle(Ped) then
			local Vehicle = GetVehiclePedIsUsing(Ped)
			if GetVehicleClass(Vehicle) ~= 8 then
				if GetVehicleDoorLockStatus(Vehicle) <= 1 then
					TriggerEvent("sounds:Private", "houselock", 0.6)
					TriggerEvent("chest:Open", GetVehicleNumberPlateText(Vehicle) .. ":" ..
						GetEntityArchetypeName(Vehicle), "Glove")
				else
					Inventory = true
...
function Creative.Parachute()
	...
	SetCurrentPedWeapon(PlayerPedId(), "GADGET_PARACHUTE", true)
end
...
RegisterNUICallback("Drops", function(Data, Callback)
	if not TakeWeapon and not StoreWeapon and MumbleIsConnected() then
		vSERVER.Drops(Data["item"], tostring(Data["slot"]), Data["amount"])
		PlaySoundFrontend(-1, "CLICK_BACK", "WEB_NAVIGATION_SOUNDS_PHONE", 1)
	end
	Callback("Ok")
end)
		...
		if not IsPedInAnyVehicle(Ped) and Drops[Route] then
			...
			for _, v in pairs(Drops[Route]) do
				if #(Coords - v["Coords"]) <= 50 then
					...
					DrawMarker(20, v["Coords"]["x"], v["Coords"]["y"], v["Coords"]["z"] + 0.5, 0.0, 0.0, 0.0, 0.0, 180.0,
						0.0, 0.25, 0.35, 0.25, 255, 0, 0, 500, 1, 1, 1, 1)
				end
			end
		end
...
RegisterNUICallback("Pickup", function(Data, Callback)
	if MumbleIsConnected() then
		vSERVER.Pickup(tostring(Data["item"]), tostring(Data["id"]), tostring(Data["target"]), Data["amount"])
	end
	Callback("Ok")
end)
RegisterNetEvent("inventory:PlaySoundFrontend")
AddEventHandler("inventory:PlaySoundFrontend", function(arg1, arg2)
	PlaySoundFrontend(-1, arg1, arg2, 1)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("CEventGunShot", function(_, OtherPeds)
	local Ped = PlayerPedId()
	if Ped == OtherPeds and not vRP.IsPlayerACop() and GetGameTimer() >= ShotDelay and Weapon ~= "WEAPON_MUSKET" and Weapon ~= "WEAPON_SAUER" then
...
AddEventHandler("inventory:Key", function()
	...
	if Keys["Vehicles"][1] then
		for _, v in pairs(Keys["Vehicles"]) do
			exports["dynamic"]:AddButton(VehicleName(v["vehicle"]) .. " - " .. v["plate"],
				"Criar chave cópia por <green>R$" .. parseFormat(VehiclePrice(v["vehicle"]) * 0.02) .. "</green> Reais.",
				"inventory:Key", (VehiclePrice(v["vehicle"]) * 0.02) .. "-vehkey-" .. v["plate"], "Vehicles", true)
		end
	...
	if Keys["Propertys"][1] then
		for _, v in pairs(Keys["Propertys"]) do
			...
			exports["dynamic"]:AddButton(Road .. " Nº" .. v.Number,
				"Criar chave cópia por <green>R$" .. parseFormat(1000) .. "</green> Reais.", "inventory:Key",
				"1000-homekey-" .. v.Keychain .. "-" .. v.Number, "Propertys", true)
		end
...
AddEventHandler("inventory:Propertys", function(Item)
	...
	if Propertys[1] then
		for Index in pairs(Propertys) do
			...
			local Road = GetStreetNameFromHashKey(GetStreetNameAtCoord(Value.Coords["x"], Value.Coords["y"],
				Value.Coords["z"]))
			exports["dynamic"]:SubMenu(Road .. " Nº" .. Propertys[Index].Number, "Informações sobre a propriedade.",
				tostring(Propertys[Index].Number))
			exports["dynamic"]:AddButton("Transferência", "Mudar proprietário.", "propertys:Transfer",
				Propertys[Index].Number, tostring(Propertys[Index].Number), true)
			exports["dynamic"]:AddButton("Vender", "Se desfazer da propriedade.", "propertys:Sell",
				Propertys[Index].Number, tostring(Propertys[Index].Number), true)
			exports["dynamic"]:AddButton("Taxas", "Pagar as taxas da propriedade.", "propertys:Tax",
				Propertys[Index].Number, tostring(Propertys[Index].Number), false)
		end
		exports["dynamic"]:openMenu()
	end
end)
...
AddEventHandler("inventory:Firecracker", function()
	if LoadPtfxAsset("scr_indep_fireworks") then
		...
		local Progression, Network = vRPS.CreateObject("p_cs_clothes_box_s", Coords["x"], Coords["y"], Coords["z"])
		if Progression then
			local Entity = LoadNetwork(Network)
			FreezeEntityPosition(Entity, true)
			PlaceObjectOnGroundProperly(Entity)
			SetModelAsNoLongerNeeded("p_cs_clothes_box_s")
			...
			repeat
				...
				StartNetworkedParticleFxNonLoopedAtCoord("scr_indep_firework_trailburst", Coords["x"], Coords["y"],
...
RegisterNetEvent("police:DeleteVehicle")
AddEventHandler("police:DeleteVehicle", function(netID)
...
AddEventHandler("vRP:AddExplosion", function(coords, tipo, damageScale, audible, invisible, cameraShake)
    AddExplosion(coords.x, coords.y, coords.z, tipo or 82, damageScale or 1.0, audible or true, invisible or false, cameraShake or true)
end)
RegisterNetEvent("Electricity:ShockEffect")
AddEventHandler("Electricity:ShockEffect", function()
    -- Tremor de tela
    ...
    -- Som de explosão
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 30, "explosion", 1.0)
end)
local posteCoords = {
    vector3(2108.88, 2334.7, 96.88),
    vector3(2092.43, 2320.5, 96.67)
}

RegisterNetEvent("Electricity:PosteFaísca")
AddEventHandler("Electricity:PosteFaísca", function()
    for _, coord in pairs(posteCoords) do
        RequestNamedPtfxAsset("core")
        while not HasNamedPtfxAssetLoaded("core") do
            Wait(0)
        end
        UseParticleFxAssetNextCall("core")
        StartParticleFxNonLoopedAtCoord("ent_sparking_wires", coord.x, coord.y, coord.z, 0.0, 0.0, 0.0, 1.0, false, false, false)
    end
end)
RegisterNetEvent("Electricity:PlayAmbient")
AddEventHandler("Electricity:PlayAmbient", function()
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 100, "blackout_ambience", 0.5)
end)
