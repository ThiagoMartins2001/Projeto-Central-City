...
function Creative.Dismantle()
	if not Active then
		...
		TriggerEvent("NotifyPush",
			{ code = 20, title = "Localização Veículo", x = Dismantle[Selected]["x"], y = Dismantle[Selected]["y"], z =
			Dismantle[Selected]["z"], color = 44 })
...
AddEventHandler("dismantle:Dispatch", function()
	...
	for Number = 1, Amounts do
		...
		if HitZ and HitSafe then
			local Application, Network = vRPS.CreatePed(Peds[OtherPeds], SafeCoords["x"], SafeCoords["y"],
				SafeCoords["z"])
			if Application then
