...
-----------------------------------------------------------------------------------------------------------------------------------------
Use = {
	["radar"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		TriggerEvent("radarilegal:createRadar", source, Passport, Amount, Slot, Full, Item, Split)
	end,
	["teddy"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		Active[Passport] = os.time() + 10
		Player(source)["state"]["Buttons"] = true
		TriggerClientEvent("inventory:Close", source)
		vRPC.createObjects(source, "timetable@trevor@skull_loving_bear", "skull_loving_bear", "prop_mr_raspberry_01", 49,
			28422)
		repeat
	...
	["dildo"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		Active[Passport] = os.time() + 10
		Player(source)["state"]["Buttons"] = true
		TriggerClientEvent("inventory:Close", source)
		vRPC.createObjects(source, "switch@trevor@jerking_off", "trev_jerking_off_loop", "prop_cs_dildo_01", 49, 28422,
			0.09, 0.05, -0.03, 90.0, 80.0, 30.0)
		repeat
			if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
				vRPC.removeObjects(source)
				Active[Passport] = nil
	...
	["tenda"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		Player(source)["state"]["Buttons"] = true
		TriggerClientEvent("inventory:Close", source)
	...
	["drugtable"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if not vRP.InsideVehicle(source) then
			if not vCLIENT.ReturnWeapon(source) then
				TriggerClientEvent("inventory:Close", source)
				local Hash = "penhashop_mesa_v1_09"
				local application, Coords, heading = vRPC.objectCoords(source, Hash)
				if application then
					if not vCLIENT.objectExist(source, Coords, Hash) then
						if vRP.TakeItem(Passport, Full, 1, true, Slot) then
	...
	["adrenaline"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		if Total <= 0 then
			...
			if #(Coords - vec3(-471.10,6288.58,13.61)) <= 20 then
	...
	["jackham"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		for i = 1, 2 do
			...
			table.insert(Coordinates,
				{ x = x, y = y, item = (i % 2 ~= 0) and "sapphireore" or "ironore", amount = (i % 2 ~= 0) and 4 or 8 })
		end
			...
			table.insert(Coordinates,
				{
					x = x,
					y = y,
					item = (i % 2 ~= 0) and "rubyore" or "diamondore",
					amount = (minutes % 2 ~= 0) and 8 or
						10
				})
		end
		...
		for i = 1, 8 do
			...
			table.insert(Coordinates,
				{ x = x, y = y, item = (i % 2 ~= 0) and "goldore" or "copperore", amount = (i % 2 ~= 0) and 3 or 5 })
		end
		...
		for i = 1, 12 do
			...
			table.insert(Coordinates,
				{ x = x, y = y, item = (i % 2 ~= 0) and "leadore" or "emeraldore", amount = (minutes % 2 ~= 0) and 5 or 7 })
		end
		...
		if #(Coords - vec3(mineCoords[1], mineCoords[2], mineCoords[3])) <= 30 then
			if vRP.ConsultItem(Passport, "drillbit", 1) then
				...
				vRPC.createObjects(source, "amb@world_human_const_drill@male@drill@base", "base", "prop_tool_jackham", 15,
					28422)
				repeat
				...
				TriggerClientEvent("Notify", source, "amarelo", "Precisa de <b>1x " .. itemName("drillbit") .. "</b>.",
					5000)
			end
	...
	["pickaxe"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		for i = 1, 2 do
			...
			local y = mineCoords[2] + 7 * math.sin(angle) + 3
			table.insert(Coordinates,
				{ x = x, y = y, item = (i % 2 ~= 0) and "sapphireore" or "ironore", amount = (i % 2 ~= 0) and 4 or 3 })
		end
		...
		for i = 1, 4 do
			...
			local y = mineCoords[2] + 14 * math.sin(angle) + 3
			table.insert(Coordinates,
				{ x = x, y = y, item = (i % 2 ~= 0) and "rubyore" or "diamondore", amount = (minutes % 2 ~= 0) and 3 or 2 })
		end
		...
		for i = 1, 8 do
			...
			local y = mineCoords[2] + 21 * math.sin(angle) + 3
			table.insert(Coordinates,
				{ x = x, y = y, item = (i % 2 ~= 0) and "goldore" or "copperore", amount = (i % 2 ~= 0) and 5 or 3 })
		end
		...
		for i = 1, 12 do
			...
			local y = mineCoords[2] + 28 * math.sin(angle) + 1
			table.insert(Coordinates,
				{ x = x, y = y, item = (i % 2 ~= 0) and "leadore" or "emeraldore", amount = (minutes % 2 ~= 0) and 1 or 2 })
		end
		...
		if #(Coords - vec3(mineCoords[1], mineCoords[2], mineCoords[3])) <= 30 then
			...
			vRPC.createObjects(source, "melee@large_wpn@streamed_core", "ground_attack_on_spot", "prop_tool_pickaxe", 1,
				18905, 0.10, -0.1, 0.0, -92.0, 260.0, 5.0)
			repeat
				if os.time() >= parseInt(Active[Passport]) then
					Active[Passport] = nil
					for _, Value in ipairs(Coordinates) do
	...
	["bandage"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if (not Healths[Passport] or os.time() > Healths[Passport]) then
			...
			TriggerClientEvent("Notify", source, "amarelo",
				"Aguarde <b>" .. Healths[Passport] - os.time() .. "</b> segundos.", 5000, "Atenção")
		end
	end,
	...
	["camera2"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if not Player(source)["state"]["Camera"] then
			...
			if math.random(100) <= 50 then
				vRPC.createObjects(source, "chocoholic@single110a", "single110a_clip", "prop_ing_camera_01", 49, 28422,
					0.0710, 0.0150, -0.0420, -68.3220, 99.6144, 2.9027)
			elseif math.random(100) <= 50 then
				vRPC.createObjects(source, "cellphone@female", "cellphone_text_read_base_cover_low", "prop_ing_camera_01",
					49, 28422, 0.0100, -0.0300, 0.0520, -172.0487, -163.9389, -29.0221)
			else
				vRPC.createObjects(source, "chocoholic@single110b", "single110b_clip", "prop_ing_camera_01", 49, 60309,
	...
	["facechange"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if vRP.TakeItem(Passport, Full, 1, false, Slot) then
			module("vrp", "lib/Proxy").getInterface("nation_creator")._startCreator(source)
			TriggerClientEvent("inventory:Close", source)
		end
	end,
	...
	["badge01"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		local Consult = exports["oxmysql"]:query_async("SELECT * FROM smartphone_instagram WHERE user_id = @Passport;",
			{ Passport = Passport })
		local Groups = vRP.Groups()
		for k, v in pairs(Groups) do
			if Groups[k].Type and Groups[k].Type == "Police" then
				local Datatable = vRP.GetSrvData("Permissions:" .. k)
				if Datatable[tostring(Passport)] then
					TriggerClientEvent("inventory:Close", source)
					vRPC.createObjects(source, "paper_1_rcm_alt1-8", "player_one_dual-8", "denis3d_policebadge_01", 49,
						28422, 0.13, 0.05, -0.05, 40.0, 55.0, -267.0)
					...
					for _, OtherSource in pairs(ClosestPeds) do
						TriggerClientEvent("badge:Show", OtherSource, Rank, Identity["name"] .. " " .. Identity["name2"],
							Passport, Avatar, k)
					end
					return
				end
	...
	["lsd"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		Active[Passport] = os.time() + 5
		Player(source)["state"]["Buttons"] = true
		TriggerClientEvent("inventory:Close", source)
		TriggerClientEvent("Progress", source, 5000)
	...
	["lanca"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		TriggerClientEvent("inventory:Close", source)
		TriggerClientEvent("Progress", source, 5000)
		vRP.createObjects(source, "amb@incar@male@smoking@enter", "enter", "mah_lanca", 48, 28422, 0.0, 0.01, 0.0, -10.0,
			-10.0, 0.0)
		repeat
			if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
				vRPC.removeObjects(source)
				Active[Passport] = nil
				if vRP.TakeItem(Passport, Full, 1, true, Slot) then
					TriggerClientEvent("Lanca", source)
					vRP.ChemicalTimer(Passport, 120)
					vRP.DowngradeStress(Passport, 20)
				end
			end
	...
	["meth"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if Armors[Passport] and os.time() < Armors[Passport] then
			TriggerClientEvent("Notify", source, "amarelo", "Aguarde <b>" ..
				Armors[Passport] - os.time() .. "</b> segundos.", 5000, "Aviso")
			return
		end
		Active[Passport] = os.time() + 15
		Player(source)["state"]["Buttons"] = true
	...
	["cigar"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if vRP.ConsultItem(Passport, "lighter", 1) then
			...
			TriggerClientEvent("inventory:Close", source)
			TriggerClientEvent("Progress", source, 10000)
			vRP.createObjects(source, "amb@world_human_smoking@male@male_a@enter", "enter", "prop_cigar_01", 48, 28422,
				0.0, 0.01, 0.0, -10.0, -10.0, 0.0)
	...
	["vape"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		vRPC.createObjects(source, "amb@world_human_smoking@male@male_b@base", "base", "xm3_prop_xm3_vape_01a", 49, 28422,
			-0.02, -0.02, 0.02, 58.0, 110.0, 10.0)
				...
				if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
					...
					if vRP.TakeItem(Passport, Full, 1, true, Slot) then
						...
						if Informations then
							...
							TriggerClientEvent("Notify", source, "verde", Message, 10000, "Informações")
						else
					...
					if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
						...
						if vRP.TakeItem(Passport, Full, 1, true, Slot) then
							...
							TriggerClientEvent("Notify", source, "azul",
								"<b>Químicos:</b> " ..
								chemStr .. "<br><b>Álcool:</b> " .. alcoholStr .. "<br><b>Drogas:</b> " .. weedStr, 8000,
								"Informações")
						end
					end
	...
	["homekey"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		local Number = exports["propertys"]:Proximity(Coords) or
			(exports["propertys"]:Exit(Coords) and parseInt(GetPlayerRoutingBucket(source) - 100000))
		if Number then
			vRPC.createObjects(source, "anim@heists@keycard@", "exit", "h4_prop_h4_key_desk_01", 48, 57005, 0.15, 0.05,
				-0.01, 30.0, 60.0, -90.0)
				...
				if vRP.ConsultItem(Passport, "screwdriver", 1) then
					...
					if vTASKBAR.Task(source, 3, 10000) then
						if vRP.TakeItem(Passport, Full, 1, true) then
							...
							TriggerClientEvent("Notify", source, "amarelo", "Substituição da fechadura concluída.", 5000,
								"Atenção")
						end
					end
					vRPC.removeObjects(source)
				else
					TriggerClientEvent("Notify", source, "amarelo", "Precisa de <b>1x " ..
						itemName("screwdriver") .. "</b>.", 5000, "Atenção")
				end
			end
		end
	end,
		...
		if not Player(source)["state"]["Handcuff"] then
			...
			if Vehicle then
				...
				if Number then
					...
					vRPC.playAnim(source, true,
						Doors[Number]["Other"] and { "veh@mower@base", "start_engine" } or
						{ "anim@heists@keycard@", "exit" }, false)
	...
	["mushroomtea"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		repeat
			if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
				...
				Player(source)["state"]["Buttons"] = false
				if vRP.TakeItem(Passport, Full, 1, true, Slot) then
					TriggerClientEvent("Energetic", source, 30, 2.20)
					vRP.UpgradeThirst(Passport, 10)
					vRP.UpgradeStress(Passport, 5)
				end
			end
	...
	["guarananatural"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		TriggerClientEvent("inventory:Close", source)
		TriggerClientEvent("Progress", source, 10000)
		vRPC.createObjects(source, "amb@world_human_drinking@coffee@male@idle_a", "idle_c", "prop_food_bs_juice02", 49,
			28422, 0.0, -0.01, -0.15, 0.0, 0.0, 0.0)
	...
	["barricada4"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		local Hash = "prop_facgate_03post"
		local application, Coords, heading = vRPC.objectCoords(source, Hash)
		if application then
			if not vCLIENT.objectExist(source, Coords, Hash) then
				if vRP.TakeItem(Passport, Full, 1, true, Slot) then
		...
		if vCLIENT.IsInWater(source) then
			if vRP.TakeItem(Passport, "bait", 1, true) then
				...
				if not vRPC.PlayingAnim(source, "amb@world_human_stand_fishing@idle_a", "idle_c") then
					vRPC.AnimActive(source)
					vRPC.createObjects(source, "amb@world_human_stand_fishing@idle_a", "idle_c", "prop_fishing_rod_01",
						49, 60309)
				end
				...
				if vTASKBAR.Task(source, AmountTasks, Velocity) then
					local List = { "octopus", "shrimp", "carp", "horsefish", "tilapia", "codfish", "catfish",
						"goldenfish", "pirarucu", "pacu", "tambaqui", "anchovy","catfish","herring","orangeroughy","salmon","sardine","smalltrout","yellowperch" }
					...
					if (vRP.InventoryWeight(Passport) + itemWeight(Selected)) <= vRP.GetWeight(Passport) then
						vRP.PutExperience(Passport, "Fisherman", 15)
						vRP.GenerateItem(Passport, Selected, 5, true)
					end
				end
	...
	["apple"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		TriggerClientEvent("inventory:Close", source)
		TriggerClientEvent("Progress", source, 5000)
		vRPC.createObjects(source, "mp_player_inteat@burger", "mp_player_int_eat_burger", "sf_prop_sf_apple_01b", 49,
			60309, 0.0, 0.0150, -0.0200, -124.6964, -166.5760, 8.4572)
		repeat
			if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
				Active[Passport] = nil
				vRPC.removeObjects(source, "one")
	...
	["cannedsoup"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if vRP.ConsultItem(Passport, "canopener", 1) then
			...
			if vRP.ConsultItem(Passport, "spoon", 1) then
				...
				TriggerClientEvent("Progress", source, 10000)
				vRPC.playAnim(source, true,
					{ "anim@scripted@island@special_peds@pavel@hs4_pavel_ig5_caviar_p1", "base_idle" }, true)
				local Ped = GetPlayerPed(source)
				...
				TriggerClientEvent("AttachEntityToEntity", source, NetworkGetNetworkIdFromEntity(Can),
					NetworkGetNetworkIdFromEntity(Ped), 60309, -0.03, 0.015, 0.06, -20.0, 30.0, 180.0)
				local Spoon = CreateObject(GetHashKey("h4_prop_h4_caviar_spoon_01a"), GetEntityCoords(Ped), true, true,
					false)
				while not DoesEntityExist(Spoon) do Wait(0) end
				TriggerClientEvent("AttachEntityToEntity", source, NetworkGetNetworkIdFromEntity(Spoon),
					NetworkGetNetworkIdFromEntity(Ped), 28422, 0.0, 0.01, 0.0, -10.0, -10.0, 0.0)
				repeat
					if os.time() >= parseInt(Active[Passport]) then
						Active[Passport] = nil
				...
				while not DoesEntityExist(Can) do Wait(0) end
				TriggerClientEvent("AttachEntityToEntity", source, NetworkGetNetworkIdFromEntity(Can),
					NetworkGetNetworkIdFromEntity(Ped), 57005, 0.13, 0.05, -0.02, -90.0, -72.0, -23.0)
				repeat
					if os.time() >= parseInt(Active[Passport]) then
						Active[Passport] = nil
						while DoesEntityExist(Can) do
			...
			TriggerClientEvent("Notify", source, "amarelo", "Precisa de <b>1x " .. itemName("canopener") .. "</b>.", 5000,
				"Atenção")
		end
	end,
	...
	["tyres"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if not vRP.InsideVehicle(source) then
			...
			if Vehicle then
				...
				vRPC.createObjects(source, "anim@heists@box_carry@", "idle", "imp_prop_impexp_tyre_01a", 49, 28422, -0.02,
	...
	["platepremium"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		if Vehicle and Network and Plate and Model then
			...
			if vRP.Query("vehicles/selectVehicles", { Passport = Passport, vehicle = Model })[1] then
				...
				if Keyboard then
					local plate = sanitizeString(Keyboard[1],
					...
					if string.len(plate) ~= 8 then
						...
						if vRP.PassportPlate(plate) then
							TriggerClientEvent("Notify", source, "amarelo", "Placa escolhida já existe no sistema.", 5000,
								"Aviso")
							return
						else
							if vRP.TakeItem(Passport, Full, 1, true, Slot) then
								SetVehicleNumberPlateText(NetworkGetEntityFromNetworkId(Network), string.upper(plate))
								vRP.Query("vehicles/plateVehiclesUpdate",
									{ Passport = Passport, vehicle = Model, plate = string.upper(plate) })
	...
	["radio"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		TriggerClientEvent("inventory:Close", source)
		TriggerClientEvent("Codinradio:openradio", source)
		vRPC.AnimActive(source)
	end,
	...
	--[["laptop"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		local permis = false
		if vRP.HasPermission(Passport, "Pmerj") and vRP.HasService(Passport, "Pmerj") then
			permis = true
		elseif vRP.HasPermission(Passport, "Admin") and vRP.HasService(Passport, "Admin") then
			permis = true
		elseif vRP.HasPermission(Passport, "DHC") and vRP.HasService(Passport, "DHC") then
			permis = true
		elseif vRP.HasPermission(Passport, "Pcerj") and vRP.HasService(Passport, "Pcerj") then
			permis = true
		elseif vRP.HasPermission(Passport, "Core") and vRP.HasService(Passport, "Pcerj") then
			permis = true
		elseif vRP.HasPermission(Passport, "Exercito") and vRP.HasService(Passport, "Exercito") then
			permis = true
		elseif vRP.HasPermission(Passport, "Bope") and vRP.HasService(Passport, "Bope") then
			permis = true
		elseif vRP.HasPermission(Passport, "PoliceFederal") and vRP.HasService(Passport, "PoliceFederal") then
			permis = true
		elseif vRP.HasPermission(Passport, "Core") and vRP.HasService(Passport, "Core") then
			permis = true
		elseif vRP.HasPermission(Passport, "Prf") and vRP.HasService(Passport, "Prf") then
			permis = true
		elseif vRP.HasPermission(Passport, "Choque") and vRP.HasService(Passport, "Choque") then
			permis = true
		elseif vRP.HasPermission(Passport, "Recom") and vRP.HasService(Passport, "Recom") then
			permis = true
		end
		if permis then
			TriggerClientEvent("inventory:Close", source)
			...
			TriggerClientEvent("Notify", source, "amarelo", "Precisa ter permissao.", 5000, "Atenção")
		end
	end,]]
	...
	["paymentterminal"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		TriggerClientEvent("inventory:Close", source)
		vRPC.AnimActive(source)
		vRPC.createObjects(source, 'cellphone@', 'cellphone_text_read_base', "bzzz_prop_payment_terminal", 49, 57005,
			0.18, 0.01, 0.0, -54.0, 220.0, 43.0, false, false, false, false, 1, true)
	end,
			...
			if Keyboard then
				if vRP.Request(source, "Pagamento", "Por favor, confirme se autoriza a transação de <b>R$" .. parseFormat(Keyboard[1]) .. "</b> reais em seu cartão de crédito.") then
					if vRP.PaymentCredit(Passport, parseInt(Keyboard[1]), Keyboard[2]) then
						vRP.GiveBank(OtherPassport, parseInt(Keyboard[1])) -- Corrigido para usar OtherPassport
						vRP.GiveItem(Passport, "receipt-" .. Passport ..
							"-" .. os.time() .. "-" .. Keyboard[1] .. "-" .. Keyboard[2], 1, true)
			...
			if Keyboard then
				if vRP.Request(source, "Pagamento", "Por favor, confirme se autoriza a transação de <b>R$" .. parseFormat(Keyboard[1]) .. "</b> reais em seu cartão de débito.") then
					if vRP.PaymentBank(Passport, parseInt(Keyboard[1]), Keyboard[2]) then
						...
						TriggerClientEvent("Notify", OtherSource, "verde", "Compra aprovada.", 5000)
					else
						TriggerClientEvent("Notify", source, "vermelho", "<b>Cartão</b> recusado.", 5000, "Liso")
						TriggerClientEvent("Notify", OtherSource, "vermelho", "<b>Cartão</b> recusado.", 5000, "Liso")
					end
	...
	["rope"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if not vRP.InsideVehicle(source) then
			if not Carry[Passport] then
				local OtherSource = vRPC.ClosestPed(source, 2)
				local OtherPassport = vRP.Passport(OtherSource)
				if OtherSource and not Carry[OtherPassport] and not vRP.IsEntityVisible(OtherSource) then
					if vRP.GetHealth(OtherSource) > 100 then
						local Identity = vRP.Identity(Passport)
						if vRP.Request(OtherSource, "Deseja aceita ser carregado por <b>" .. Identity["name"] .. " " .. Identity["name2"] .. "</b> ?") then
							...
							vRPC.playAnim(OtherSource, false, { "nm", "firemans_carry" }, true)
						else
							TriggerClientEvent("Notify", source, "vermelho", "A pessoa negou ser carregado.", 3000)
						end
					else
						Carry[Passport] = OtherSource
						Player(OtherSource)["state"]["Carry"] = true
				...
				if DoesPlayerExist(Carry[Passport]) then
					...
					vRPC.stopAnim(Carry[Passport], false)
				end
				vRPC.stopAnim(source, false)
				Carry[Passport] = nil
			end
			...
			if OtherPassport then
					...
					if Value["Type"] == "Police" then
						if vRP.HasService(OtherPassport, key) then
							...
							if vRP.TakeItem(Passport, Full, 1, true, Slot) then
								...
								TriggerClientEvent("Notify", source, "verde", "Todas as comunicações foram retiradas.",
									5000, "Sucesso")
							end
						end
					end
				...
				if vRP.HasService(OtherPassport, "Hospital") then
					...
					if vRP.TakeItem(Passport, Full, 1, true, Slot) then
						...
						TriggerClientEvent("Notify", source, "verde", "Todas as comunicações foram retiradas.", 5000,
							"Sucesso")
					end
				end
			end
