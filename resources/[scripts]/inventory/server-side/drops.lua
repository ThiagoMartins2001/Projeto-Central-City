...
function Creative.Drops(Item,Slot,Amount)
	...
	if Passport and not Active[Passport] and Amount > 0 and not Player(source)["state"]["Handcuff"] and not exports["hud"]:Wanted(Passport) and not vRPC.InsideVehicle(source) then
		...
		if not itemBlock(Item) then
			...
			local webdrops = "https://discordapp.com/api/webhooks/1281316402573344818/PX4cN2tiMn_wPhNYfNJQcRCqKe7d1kOiEdZz6xbYPuHcVsJ8k6RV470zcFt4YEDAIBSE"
...
-- DROPSERVER
-----------------------------------------------------------------------------------------------------------------------------------------
function Creative.DropServer(Route,Coords,Item,Amount)
...
-- PICKUP
-----------------------------------------------------------------------------------------------------------------------------------------
function Creative.Pickup(Item,Number,Target,Amount)
	local source = source
	local Amount = parseInt(Amount,true)
	local Passport = vRP.Passport(source)
	local Route = GetPlayerRoutingBucket(source)
