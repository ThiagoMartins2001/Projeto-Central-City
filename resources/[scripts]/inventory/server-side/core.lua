		...
		for Index, v in pairs(Inv) do
			if not v["item"] or (parseInt(v["amount"]) <= 0 or not itemBody(v["item"])) then
				...
				if Split[1] == "identity" or Split[1] == "fidentity" or string.sub(v["item"], 1, 5) == "badge" then
					...
					v["desc"] = v["desc"] ..
						"<br><legenda>Passaporte: <r>" ..
						Number ..
						"</r> <br>Nome: <r>" .. (Identity["name"] .. " " .. Identity["name2"]) .. "</r></legenda>"
				...
				elseif Split[1] == "creditcard" then
					...
					local Identity = vRP.Identity(Number)
					v["desc"] = v["desc"] ..
						"<br><legenda>Nome: <r>" .. (Identity["name"] .. " " .. Identity["name2"]) .. "</r></legenda>"
				...
				elseif Split[1] == "driverlicense" then
					v["desc"] = v["desc"] ..
						"<br><legenda>Nome: <r>" ..
						json.decode(Split[3])["name"] ..
						"</r> <br>Emissão: <r>" ..
						os.date("%d/%m/%Y", json.decode(Split[3])["issued"]) ..
						"</r><br>Validity: <r>" ..
						os.date("%d/%m/%Y", json.decode(Split[3])["expiration"]) ..
						"</r><br>Categoria: <r>" ..
						string.gsub(json.encode(json.decode(Split[3])["categories"]), '[^%a,]', '') .. "</r></legenda>"
				elseif Split[1] == "dmvdocs" then
					v["desc"] = v["desc"] ..
						"<br><legenda>Nome: <r>" ..
						vRP.Identity(parseInt(Split[2]))["name"] ..
						" " ..
						vRP.Identity(parseInt(Split[2]))["name2"] ..
						"</r> <br>Prática: <r>" ..
						string.gsub(Split[5], '[^%a,]', '') ..
						"</r><br>Teórica: <r>" ..
						string.gsub(Split[4], '[^%a,]', '') ..
						"</r><br>Categoria: <r>" .. string.gsub(Split[3], '[^%a,]', '') .. "</r></legenda>"
				...
				elseif Split[1] == "receipt" then
					v["desc"] = v["desc"] ..
						"<br><legenda>Data: <r>" ..
						os.date("%Y-%m-%d %H:%M:%S", Split[3]) ..
						"</r> <br>Nome: <r>" ..
						vRP.Identity(parseInt(Split[2]))["name"] ..
						" " ..
						vRP.Identity(parseInt(Split[2]))["name2"] ..
						"</r><br>Valor: <r>" .. parseFormat(Split[4]) .. "</r><br>Descrição: <r>" ..
						Split[5] .. "</r></legenda>"
				end
...
-- SEND
-----------------------------------------------------------------------------------------------------------------------------------------
function Creative.Send(Slot, Amount, Item)
	local source = source
			...
			if ClosestPed then
				...
				if OtherPassport then
					...
					if AmountAut ~= Amount and type(AmountAut) == "number" then
						Amount = Amount - AmountAut
						local Identity = vRP.Identity(Passport)
						local Identity2 = vRP.Identity(OtherPassport)
						local webdrops =
						"https://discordapp.com/api/webhooks/1281316342078636076/glkq4TVbeNcQnsvzDV44X1VISUyzKp-KD8RCGuhaGC5VLd1bW_VMtl2AIjb9ltGcDIJY"
						SendWebhookMessage(webdrops,
							"```" ..
...
function Creative.Use(Slot, Amount)
	...
	if Passport and not Active[Passport] then
		...
		if itemDurability(Full) and vRP.CheckDamaged(Full) then
			TriggerClientEvent("Notify", source, "amarelo", "<b>" .. itemName(Item) .. "</b> danificado.", 5000,
				"Atenção")
			return
		end
		...
		local Identity = vRP.Identity(Passport)
		local webdrops =
		"https://discordapp.com/api/webhooks/1281316473012355184/k1Teip8EZsiV9eOFsNGKo9HfDRacE-fxu3VjRDUCFb8FW8WIRnAYUKOP4kO0aie0B_kn"
		SendWebhookMessage(webdrops,
			"```" ..
			(Identity["name"] .. " " .. Identity["name2"]) ..
			" [" ..
			...
			"(" ..
			itemName(Full) ..
			") na cds (" .. mathLength(Coords.x) .. "," .. mathLength(Coords.y) .. "," .. mathLength(Coords.z) .. ").```")
		...
		if itemType(Full) == "Armamento" and parseInt(Slot) <= 5 then
			if not Player(source)["state"]["Safezone"] then
				...
				TriggerClientEvent("Notify", source, "amarelo", "O armamento não pode ser usado na safezone.", 5000,
					"Atenção")
			end
			...
			if Weapon then
				if vCLIENT.CheckAttachs(source, Item, Weapon) then
					...
					if not Attachs[Passport][Weapon][Item] then
						...
						TriggerClientEvent("Notify", source, "amarelo", "O armamento já possui um componente equipado.",
							5000, "Atenção")
					end
				else
			...
			if #(ParkingMeters[Index]["Coords"] - Coords) <= 0.75 then
				if os.time() <= ParkingMeters[Index]["Timer"] then
					TriggerClientEvent("Notify", source, "amarelo",
						"Aguarde <b>" .. ParkingMeters[Index]["Timer"] - os.time() .. "</b> segundos.", 5000, "Atenção")
					return
				else
					Number = Index
					break
	...
	if Passport and Loots[Box] then
		...
		if Loots[Box]["Players"][Number][Passport] then
			if os.time() <= Loots[Box]["Players"][Number][Passport] then
				TriggerClientEvent("Notify", source, "amarelo",
					"Aguarde <b>" .. Loots[Box]["Players"][Number][Passport] - os.time() .. "</b> segundos.", 5000,
					"Atenção")
				return false
			end
		end
			...
			if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
				...
				if not Plates[Plate] then
					...
					if math.random(100) >= 50 then
						...
						TriggerClientEvent("Notify", source, "amarelo", "Após remove-la a mesma quebrou.", 5000,
							"Atenção")
					end
					Plates[Plate] = nil
				end
...
AddEventHandler("inventory:Pumpjack", function(Entity)
	...
	if Passport and not Active[Passport] and Coords then
		for Index = 1, #Pumpjacks do
			if #(Pumpjacks[Index]["Coords"] - Coords) <= 0.75 then
				if os.time() <= Pumpjacks[Index]["Timer"] then
					TriggerClientEvent("Notify", source, "amarelo",
						"Aguarde <b>" .. Pumpjacks[Index]["Timer"] - os.time() .. "</b> segundos.", 5000, "Atenção")
					return
				else
					Number = Index
					break
		...
		if not vCLIENT.CheckWeapon(source, "WEAPON_CROWBAR") then
			TriggerClientEvent("Notify", source, "amarelo",
				"Você precisa colocar o <b>" .. itemName("WEAPON_CROWBAR") .. "</b> em mãos.", 5000, "Atenção")
			return
		end
...
AddEventHandler("inventory:Animals", function(Entity)
	...
	if Passport and not Active[Passport] then
		if vCLIENT.CheckWeapon(source, "WEAPON_SWITCHBLADE") then
			...
			TriggerClientEvent("Notify", source, "amarelo", "Você precisa colocar o <b>Canivete</b> em mãos.", 5000,
				"Atenção")
		end
	end
end)
				...
				if type(NeedLess) == "table" then
					for Index, v in pairs(NeedLess) do
						...
						if Consult[1] < v["amount"] then
							TriggerClientEvent("Notify", source, "amarelo",
								"Necessário possuir <b>" .. v["amount"] .. "x " .. itemName(v["item"]) .. "</b>.", 5000,
								"Atenção")
							return
						end
					...
					if Consult[1] < needAmount then
						TriggerClientEvent("Notify", source, "amarelo",
							"Necessário possuir <b>" .. needAmount .. "x " .. itemName(NeedLess) .. "</b>.", 5000,
							"Atenção")
						return
					end
				end
	...
	for Passport, _ in pairs(List) do
		if Ammos[Passport] then
			vRP.Query("playerdata/SetData",
				{ Passport = Passport, dkey = "Ammos", dvalue = json.encode(Ammos[Passport]) })
		end
...
AddEventHandler("Disconnect", function(Passport, Source)
	...
	local Ped = GetPlayerPed(Source)
	local Coords = GetEntityCoords(Ped)
	TriggerEvent("Discord", "logInvDisConnnect",
		"**SAIDA\nPassaporte:** " ..
		Passport ..
		"\n**Cds:** " ..
...
AddEventHandler("Connect", function(Passport, Source)
	...
	local Ped = GetPlayerPed(Source)
	local Coords = GetEntityCoords(Ped)
	TriggerEvent("Discord", "logInvConnnect",
		"**ENTRADA\nPassaporte:** " ..
		Passport ..
		"\n**Cds:** " ..
		mathLength(Coords["x"]) .. "," .. mathLength(Coords["y"]) .. "," .. mathLength(Coords["z"]) .. "\n" .. content,
				...
				if Value["Type"] == "Police" then
					...
					for Passports, Sources in pairs(Service) do
						async(function()
							vRPC.PlaySound(Sources, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
							TriggerClientEvent("NotifyPush", Sources,
								{ code = 10, title = Vehicle, x = Coords["x"], y = Coords["y"], z = Coords["z"], color = 6 })
						end)
					end
				end
			end
...
function Creative.CheckHouseDelay(Number)
	...
	if not Delay[Number] or Delay[Number] <= os.time() then
		...
		TriggerClientEvent("Notify", source, "amarelo",
			"Aguarde " .. CompleteTimers(tonumber(Delay[Number]) - tonumber(os.time())) .. ".", 5000, "Atenção")
		return false
	end
end
...
function Creative.UserKeys()
	...
	if Passport then
		return {
			["Vehicles"] = exports["oxmysql"]:query_async(
				"SELECT * FROM vehicles WHERE work = 'false' AND Passport = @Passport;", { Passport = Passport }),
...
AddEventHandler("inventory:Key", function(Item)
	...
	if Passport then
		...
		if vRP.PaymentFull(Passport, tonumber(Split[1]), "Keychain") then
			vRP.GiveItem(Passport,
				Split[2] == "homekey" and (Split[2] .. "-" .. Split[3] .. "-" .. Split[4]) or
				(Split[2] .. "-" .. Split[3]),
				1, true, false)
