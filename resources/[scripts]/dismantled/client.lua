...
-- CONFIG
-----------------------------------------------------------------------------------------------------------------------------------------
local IniciarServico = {1524.44,1684.23,110.11} -- Onde se inicia o serviço e verifica a existência de um carro
local LocalDesmancharCarro = {1534.13,1685.72,109.98} -- Onde deve haver o carro que será desmanchado para poder continuar o desmanche
local LocalFerramentas = {1531.27,1690.01,109.99} -- Local onde 'pegará' as ferramentas
local AnuncioChassi = {1535.43,1689.72,109.95} -- Onde finalizará a missão para entregar o chassi e receber dinheiro e itens
local Computador = {1535.49,1690.52,110.94} -- Local onde ficará o computador de venda 
---------------------------------------------------------------------
--VARIAVEIS
---------------------------------------------------------------------
local blips = {}
    ...
    while true do
        ...
        if permitido then
            ...
            if etapa == 0 then
                ...
                if dist < 10 then
                    DrawMarker(21, IniciarServico[1], IniciarServico[2], IniciarServico[3]-0.5, 0, 0, 0, 180.0, 0, 0, 0.4, 0.4, 0.4, 173, 216, 230, 150, 0, 0, 0, 1)
                    if dist < 1 then
                                ...
                                if VehPermitido then 
                                    if CheckVehPermitido(nomeCarro) then
                                        if vSERVER.CheckItem() then
                                            ...
                                            TriggerEvent('Notify','vermelho','ERRO AO INICIAR O SERVIÇO! Você necessita de uma <b>Chave Inglesa</b> para iniciar o serviço.',10000)
                                        end
                                    else
            ...
            elseif etapa == 1 then
                ...
                if dist < 10 then
                    DrawMarker(21, LocalFerramentas[1], LocalFerramentas[2], LocalFerramentas[3]-0.5, 0, 0, 0, 180.0, 0, 0, 0.4, 0.4, 0.4, 173, 216, 230 , 150, 0, 0, 0, 1)
                    if dist < 1 then
            ...
            elseif etapa == 2 then
                ...
                for k , v in pairs(PosVeh) do
                    ...
                    if not PecasRemovidas[k] then
                        ...
                        if dist < 10 then
                            DrawMarker(21, x, y, z+1, 0, 0, 0, 180.0, 0, 0, 0.4, 0.4, 0.4, 174, 2, 254 , 150, 0, 0, 0, 1)
                            if dist < 1.0 then
            ...
            elseif etapa == 3 then
                ...
                if dist < 10 then
                    DrawMarker(21, AnuncioChassi[1], AnuncioChassi[2], AnuncioChassi[3]-0.5, 0, 0, 0, 180.0, 0, 0, 0.4, 0.4, 0.4, 173, 216, 230 , 150, 0, 0, 0, 1)
                    if dist < 1 then
