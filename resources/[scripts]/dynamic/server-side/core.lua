...
-----------------------------------------------------------------------------------------------------------------------------------------
Flash = {}
Tunnel.bindInterface("dynamic",Flash)
...
AddEventHandler("dynamic:EmergencyAnnounceMedic",function()
	...
	if Passport then
		if vRP.HasGroup(Passport,"Paramedic") then
			...
			if Keyboard then
				TriggerClientEvent("Notify",-1,"blood",Keyboard[1].. " Enviado por: "..Keyboard[2],Keyboard[3],30000)
			end
		end
	end
end)
...
AddEventHandler("dynamic:Tencode",function(Code)
	...
	if Passport then
		...
		for Ped,Sources in pairs(Service) do
			async(function()
				if Code ~= 4 then
...
-- ANIMAREGISTER
-----------------------------------------------------------------------------------------------------------------------------------------
function Flash.animalRegister(netId)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- ANIMAL HASH FUNCTION
-----------------------------------------------------------------------------------------------------------------------------------------
function Flash.animalHash(hash)
	if animalHash ~= nil then
		local Passport = PlayerPedId()
		if Passport then
			animalHash[Passport] = hash
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
function Flash.dynamicPermission(source)
	local Passport = vRP.Passport(source)
	if Passport then
...
-- ANIMACLEANER
-----------------------------------------------------------------------------------------------------------------------------------------
function Flash.animalCleaner()
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
		TriggerEvent("tryDeletePed",animal[Passport])
		animal[Passport] = nil
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
