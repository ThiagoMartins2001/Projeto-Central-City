...
AddEventHandler("Taxi:initService",function(Service)
	if LocalPlayer["state"]["Route"] < 900000 then
		if LocalPlayer["state"]["wrforduber"] then	
			if DoesBlipExist(Blip) then
				RemoveBlip(Blip)
				Blip = nil
			end
			Npc = nil
			LocalPlayer["state"]:set("wrforduber",false)
			...
			LocalPlayer["state"]:set("wrforduber",true)
			routeStart()
			Salary()
...
function routeStart()
	Citizen.CreateThread(function()
	    ...
	    while LocalPlayer["state"]["wrforduber"] do
			local TimeDistance = 999
			if IsPedInAnyVehicle(Ped) and GetEntityModel(GetVehiclePedIsUsing(Ped)) == GetHashKey("wrforduber") and GetEntityHealth(Ped) > 100 then
...
function Salary()
	...
	CreateThread(function()
		while LocalPlayer["state"]["taxi"] do
			...
			local Ped = PlayerPedId()
			if IsPedInAnyVehicle(Ped)  and GetEntityHealth(Ped) > 100 then
				if GetEntityModel(GetVehiclePedIsUsing(Ped)) == GetHashKey("wrforduber") then
					if LocalPlayer["state"]["wrforduber"] then
...
RegisterCommand("testrequest", function()
    local success = pcall(function()
        exports["request"]:Request("Taxista", "Aceita Chamando?")
    end)

    if success then
        print("Função Request disponível e funcionando.")
    else
        print("Erro: Função Request indisponível.")
    end
end)
