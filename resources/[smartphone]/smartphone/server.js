...
}, pp.getUsersByPermission = c => pc('vrp_permissions').where({ 'permiss': c }).pluck('user_id'); else {
        ...
        if (pB.base === 'creative_v5') {
            ...
            for (const e of Object.keys(await pp.Players()).map(Number)) {
                if (await pp.HasPermission(e, c)) {
                    if (await pp.HasService(e, c))  d.push(e);
                }
            }
...
p6(async function () {
    try {
      ...
      console.log('^4[smartphone]^4 ^2Magnata Autenticado!^0');
        ...
        if (r.length === 0) {
            ...
            if (p.dev) {
                ...
                for (let B of r) {
                    ...
                    emitNet('NotifyPush', C, { code : 20, phone : n, name : o, title : k, x : Coords.x, y : Coords.y, z : Coords.z, criminal : p.name, time : "Recebido às ", blipColor : 16 })
                    emitNet('chatMessage', C, 'CHAMADO', [19, 197, 43], '(' + p.name + ')  Enviado por ^1' + o + '^0 [' + m + '], ' + k);
                    const D = {};
                    D.user_id = m, D.phone = n, D.name = o, emitNet('smartphone:service_request', C, D), pp.request(C,p.name,'Aceitar o chamado de <b>' + o + '</b>?').then(E => {
                        if (E) {
                          if (s) emitNet('Notify', C, 'negado', 'Esse chamado já foi atendido');
                          else {
                            s = true;
                            const H = {};
...
pF.ready(async () => {
    ...
    const e = []
    for (let [k, l, m] of [['fines', 'Message', 'Value'], ['characters_fines', 'text', 'price'], ['fines', 'motivo', 'valor'], ['rb_multas', 'motivo', 'valor'], ['vrp_multas', 'motivo', 'valor']]) {
        e.push({
            'table': k,
            ...
            'getSum'(n) {
                const p = {};
                return p.Passport = n, pc(this.table).where(p).sum(m);
            },
    ...
    i = pF.firstTable('transactions', 'vrp_bdl_banco', 'smartphone_bank');
        ...
        if (pF.hasTable('smartbank_statements')) {
            ...
            if (await pF.hasColumns('vrp_banco', 'title', 'amount', 'idtrans')) {
                ...
                if (pF.hasTable('vRP:multas')) {
                    ...
                    const x = await pc("transactions").where({ Passport : h }).orderBy('id', 'DESC').limit(100);
                    for (let y of x) {
                        y.created_at = y.Date;
                        y.description = (y.Type == 'exit' ? 'Saída' : 'Entrada') + ' de ' + pu(y.Value);
                    }
                    return x;
                }
            }
...
pL.bank_pix = async (e, f, g) => {
    ...
    if (v) {
        ...
        if (p5('requestConfirm')) {
            ...
            A = await pp.request(v,"Banco",z).catch(() => false);
        }
