...
local Garages = {
	...
	["63"] = { name = "busPolice", payment = false, perm = "Police" },
	["64"] = { name = "Prf", payment = false, perm = "Prf" },
	["65"] = { name = "Prf", payment = false, perm = "Prf" },
	["66"] = { name = "Prf", payment = false, perm = "Prf" },
	["67"] = { name = "Prf", payment = false, perm = "Prf" },
	["68"] = { name = "heliPrf", payment = false, perm = "Prf" },
	...
	["159"] = { name = "Mechanic", payment = false, perm = "Mechanic" },
	["160"] = { name = "KoiSushi", payment = false, perm = "KoiSushi" },

	--Mansoes
	["200"] = { name = "Mansao01", payment = false, perm = "Mansao01" },
	["201"] = { name = "Mansao02", payment = false, perm = "Mansao02" },
	["202"] = { name = "Mansao02", payment = false, perm = "Mansao02" },
	["203"] = { name = "Mansao03", payment = false, perm = "Mansao03" },
	["204"] = { name = "Mansao03", payment = false, perm = "Mansao03" },
	["205"] = { name = "Mansao04", payment = false, perm = "Mansao04" },
	["206"] = { name = "Mansao04", payment = false, perm = "Mansao04" },
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- SIGNALREMOVE
-----------------------------------------------------------------------------------------------------------------------------------------
...
function Creative.Tax(Name)
	...
	if Passport then
		...
		if Consult[1] and Consult[1]["tax"] <= os.time() then
			local Price = VehicleTaxPrice(Name) * 0.10

			-- 🔍 Debug: Mostra o preço base e a taxa calculada
			print(">>> [DEBUG] Veículo: "..Name)
			print(">>> [DEBUG] Preço do veículo: $"..VehiclePrice(Name))
			print(">>> [DEBUG] Taxa calculada (10%): $"..Price)

			if vRP.PaymentFull(Passport,Price_tax,TypePayment5) then
...
RegisterServerEvent("151512512512515215y")
AddEventHandler("151512512512515215y",function(user_id,car)
	local source = vRP.Source(tonumber(user_id))
	local Passport = vRP.Passport(source)
		if Passport then
...
exports("ServerSpawnEletrician",function(Passport,Model, x, y, z, Heading,Plate,source)
	local Ped = GetPlayerPed(source)
	local Coords = GetEntityCoords(Ped)
	local Heading = GetEntityHeading(Ped)
	local Plate = "VEH" .. (math.random(10000, 90000) + Passport)
	local Exist, Network, Vehicle = Creative.ServerVehicle(Model,x, y, z,
	...
	vCLIENT.CreateVehicle(-1, Model, Network, 1000, 1000, nil, false, false, { 1.25, 0.75, 0.95 },Plate)
	Spawn[Plate] = { Passport, Model, Network }
