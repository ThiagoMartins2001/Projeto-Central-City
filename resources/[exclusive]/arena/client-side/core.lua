			...
			if Distance <= 100 and LocalPlayer["state"]["Route"] < 900000 then
				if not Peds[Number] then
					if LoadModel(v["Ped"]["Model"]) then
						Peds[Number] = CreatePed(4, v["Ped"]["Model"], v["Ped"]["Coords"]["x"], v["Ped"]["Coords"]["y"],
							v["Ped"]["Coords"]["z"] - 1, v["Ped"]["Heading"], false, false)
						...
						if LoadAnim("anim@heists@heist_corona@single_team") then
							TaskPlayAnim(Peds[Number], "anim@heists@heist_corona@single_team", "single_team_loop_boss",
								8.0, 8.0, -1, 1, 0, 0, 0, 0)
						end
...
AddEventHandler("arena:Exit", function()
	if Arena ~= "0" then
		...
		SetEntityCoords(Ped, Zones[Arena]["Exit"][1], Zones[Arena]["Exit"][2], Zones[Arena]["Exit"][3], false, false,
			false, false)
...
AddEventHandler("arena:Respawn", function()
	if Arena ~= "0" then
		...
		SetEntityCoords(Ped, Zones[Arena]["Respawn"][Respawn][1], Zones[Arena]["Respawn"][Respawn][2],
			Zones[Arena]["Respawn"][Respawn][3], false, false, false, false)
			TriggerEvent("hud:active",true)
...
AddEventHandler("gameEventTriggered", function(name, Message)
	if name == "CEventNetworkEntityDamage" and Arena ~= "0" then
		if GetEntityHealth(Message[1]) <= 100 and PlayerPedId() == Message[2] and IsPedAPlayer(Message[1]) then
			SendNUIMessage({ Action = "Players", Players = Players + 2, Streek = KillStreek + 1 })
