...
local Camera = nil
local LastModel = ""
local testeautal = nil
local CurrentCoords = {}
local CoordsTest = {
	{-342.22,-1344.8,31.32,266.46},
	{1210.42,2715.13,38.0,172.92}
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- CAMERAACTIVE
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("pdm:Open",function()
	...
	if not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and GetEntityHealth(Ped) > 100 then
		...
		CurrentCoords = GetEntityCoords(Ped)
		for Key,Value in pairs(CoordsTest) do
			if 100 >= #(CurrentCoords - vec3(Value[1],Value[2],Value[3])) then
				testeautal = 2
			else
				testeautal = 1
			end
...
RegisterNUICallback("Drive",function(Data,Callback)
	if vSERVER.CheckDrive(Data) == true then
		...
		if LoadModel(Data) then
			...
			local coords = CoordsTest[testeautal]
			Mount = CreateVehicle(Data,coords[1],coords[2],coords[3],coords[4],false,false)
			...
			while true do
				...
				if not IsPedInAnyVehicle(Ped) then
					SetEntityCoords(Ped,CurrentCoords)
