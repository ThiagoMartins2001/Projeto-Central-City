...
-----------------------------------------------------------------------------------------------------------------------------------------
SnapStore  = {}
Tunnel.bindInterface("luckywheel",SnapStore )
		...
		if not IsPedInAnyVehicle(Ped) then
			...
			if Distance <= 100 then
				if not LocalPlayer["state"]["Cassino"] then
					...
					if LoadModel("s80") and LoadModel("vw_prop_vw_luckywheel_02a") then
						Wheel = CreateObjectNoOffset("vw_prop_vw_luckywheel_02a",1111.05,229.85,-49.14,false,false,false)
						SetEntityHeading(Wheel,0.0)

						Vehicle = CreateVehicle("s80",1100.04,219.87,-47.75,200.0,false,false)
						SetVehicleNumberPlateText(Vehicle,"LOYAL")
						SetVehicleOnGroundProperly(Vehicle)
						FreezeEntityPosition(Vehicle,true)
						SetEntityInvincible(Vehicle,true)
						SetVehicleDoorsLocked(Vehicle,2)
						SetVehicleColours(Vehicle,0,3)

						SetModelAsNoLongerNeeded("vw_prop_vw_luckywheel_02a")
						SetModelAsNoLongerNeeded("s80")
					end
				end
			else
				if LocalPlayer["state"]["Cassino"] then
...
-- START
-----------------------------------------------------------------------------------------------------------------------------------------
function SnapStore .Start(Result)
	if Result ~= nil then
		if LocalPlayer["state"]["Cassino"] then
			if DoesEntityExist(Wheel) then
