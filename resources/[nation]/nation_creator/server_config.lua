...
if multiCharacter then
    ...
    vRP.Prepare("nation_creator/getPlayerPosition", "SELECT position FROM characters WHERE id = @user_id")

    vRP.Prepare("nation_creator/savePlayerPosition", "UPDATE characters SET position = @position WHERE id = @user_id")
    ...
    if user_id then
        if char then
           -- vRP.setUData(user_id, "nation_char", json.encode(char,{indent=false}))
            vRP.Query("playerdata/SetData",{ Passport = user_id, dkey = "nation_char", dvalue = json.encode(char,{indent=false}) })
        end
        local sex = "M"
...
function processSpawnController(source,char,user_id)
    ...
    if char then
        ...
        TriggerEvent('Creative.loadTattoos',user_id)
    else
        userlogin[user_id] = true
...
function getUserLastPosition(source, user_id)
    local coords = {196.97,-982.64,28.30}
...
RegisterCommand('mudarsexo', function(source, args) -- COMANDO DE ADMIN PARA MUDAR O SEXO DO PERSONAGEM
    local Passport = vRP.Passport(source)
    if vRP.HasGroup(Passport, "Admin", 1) then
        if args[1] then 
            local id = tonumber(args[1])
            if id then
                local src = vRP.Source(id)
                if src and vRP.Request(source, "Deseja mudar o sexo do id "..id.." ?", "Sim", "Não") then
                    fclient._changeGender(src)
                end
            end
        elseif vRP.Request(source, "Deseja mudar seu sexo?", "Sim", "Não") then
            fclient._changeGender(source)
        end
    end
end)
...
CreateThread(function()
	while true do
		...
		--print("[AUTO SAVE] Servidor salvo automaticamente.")
	end
end)

-- Salvamento ao sair do servidor (quit ou crash)
AddEventHandler("playerDropped", function(reason)
    ...
    if Passport then
        ...
        if DoesEntityExist(ped) then
            ...
            print("Saving location for Passport:", Passport, "Coords:", coords, "Heading:", heading)

            vRP.Execute("characters/saveLocation", {
                Passport = Passport,
                x = coords.x,
                y = coords.y,
                z = coords.z,
